...
- (void)awakeFromNib
{
    ...
    self.speed = ZCFlashCtntSpeedMild;
    self.repeatCount = 0;
...
- (void)setSpeed:(ZCFlashCtntSpeed)speed
{
    _speed = speed;
    switch (_speed) {
        case ZCFlashCtntSpeedFast:
            rate = 90.;
            break;
        case ZCFlashCtntSpeedMild:
            rate = 75;
            break;
        case ZCFlashCtntSpeedSlow:
            rate = 40.;
            break;
        default:
            break;
...
- (NSAttributedString *)setAttributedTextDefaultFont:(NSAttributedString *)attrText
{
    ...
    //[rtn addAttribute:NSParagraphStyleAttributeName value:paragraphStyle range:NSMakeRange(0, rtn.string.length)];
...
- (void)reloadView
{
    ...
    CGRect f = CGRectMake(0, 0, moveNeed?width:self.bounds.size.width, self.bounds.size.height);
