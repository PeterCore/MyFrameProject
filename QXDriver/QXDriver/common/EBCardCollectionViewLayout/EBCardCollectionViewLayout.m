...
- (void)prepareLayout{
    ...
    for (NSInteger section = 0; section < sectionCount; section++) {
        ...
        for (NSInteger item = 0; item < itemCount; item++) {
            ...
            itemAttributes.frame = [self contentFrameForCardCount:count];
    ...
    [self.layoutInfo enumerateKeysAndObjectsUsingBlock:^(NSString *elementIdentifier,
        ...
        [elementsInfo enumerateKeysAndObjectsUsingBlock:^(NSIndexPath *indexPath,
            ...
            if (self.isScale) {
                ...
                //计算y方向的间距
                CGFloat deltaY = attributes.frame.origin.y - delta;
                
                CGFloat scaleY = 1 + deltaY/ self.collectionView.frame.size.height*0.1;
                
                attributes.transform = CGAffineTransformMakeScale(1, scaleY);
            }
...
- (CGSize)collectionViewContentSize {
    ...
    if (_layoutType == EBCardCollectionLayoutHorizontal) {
        ...
        for (int i = 0; i <self.collectionView.numberOfSections; i++) {
            count = count + [self.collectionView numberOfItemsInSection:i];

        }
        retVal = CGSizeMake([self pageWidth] * count + _offset.horizontal,
...
- (CGRect)contentFrameForCardCount:(NSInteger)count {
    
    CGRect retVal = CGRectZero;
    
    if (_layoutType == EBCardCollectionLayoutHorizontal) {
        NSInteger posX = _offset.horizontal / 2 + [self pageWidth] * count;
        
        if ([self.collectionView numberOfItemsInSection:0] == 1) {
            //  If there's just an only item. Center it.
            posX = _offset.horizontal + [self pageWidth] * count;
        }
        ...
        retVal = CGRectMake(_offset.horizontal,
                            _offset.vertical / 2 + [self pageHeight] * count,
                            [self cardWidth],
                            [self cardHeight]);
    }
